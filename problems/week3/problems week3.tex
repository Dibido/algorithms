...
The algorithm to find a universal sink is as follows:\\ We go through every vertex and go from left to right through the rows, the row needs to have only zeroes to be a universal sink.\\ So if we encounter a 1 on a row we go to the next row. If this row has only zeroes to the right we have a probable vertex for our universal sink.\\ To be sure we check that this vertex has no other zeroes in its column.\\
...
Alternatively we can run DFS for every vertex in G, if all other nodes are black and the starting node is still gray, then the starting node must be a mother vertex.\\
