...
\begin{document}
	...
	In this graph the shortest path in the original graph is A->B->D. When we add a constant (in this case 2) the shortest path becomes A->D. This is incorrect. This is because the path can have an uneven amount of edges. Thus we add more to the bottom path than the upper path.
	
	\subsection*{5.5}
	
	Because our residual graphs for the steps were rather messy we have redrawn the final residual graph so that it was actually readable.
	...
	The route that enables this is also seen in the residual graph.
